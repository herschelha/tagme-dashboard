...
function VisitorDetailModal({ visitor, token, onClose, onDeleted }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Delete ${visitor.name}'s details permanently? This can't be undone.`)) return;
    setDeleting(true);
    try {
      const res = await fetch(`${API}/api/customer/visitors/${visitor.id}`, {
        method: 'DELETE',
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'white',borderRadius:16,padding:28,width:'90%',maxWidth:440,boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
        ...
        <button onClick={handleDelete} disabled={deleting} style={{width:'100%',marginTop:8,padding:10,background:'transparent',border:'1px solid #fecaca',borderRadius:10,color:'#dc2626',fontWeight:600,cursor:deleting?'default':'pointer',opacity:deleting?0.6:1}}>
          {deleting ? 'Deleting...' : '🗑️ Delete This Visitor'}
        </button>
      </div>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f0f4f8'}}>
      ...
      {modal?.type === 'detail' && (
        <VisitorDetailModal visitor={modal.visitor} token={token} onClose={closeModal} onDeleted={closeModal} />
      )}
    </div>
  );
}
